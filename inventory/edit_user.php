...
if(isset($_POST['update-pass'])) {
  ...
  if(empty($errors)){
     ...
     $h_pass   = sha1($password);
       ...
       $result = $db->query($sql);
        if($result && $db->affected_rows() === 1){
          $session->msg('s',"用户密码已经更新成功 ");
          redirect('edit_user.php?id='.(int)$e_user['id'], false);
        } else {
          $session->msg('d',' 抱歉！用户密码更新失败');
          redirect('edit_user.php?id='.(int)$e_user['id'], false);
        }
  } else {
    $session->msg("d", $errors);
     ...
     <div class="panel panel-default">
       <div class="panel-heading">
        <strong>
          ...
          更新 <?php echo remove_junk(ucwords($e_user['name'])); ?> 账号
        </strong>
       </div>
  ...
  <div class="col-md-6">
    <div class="panel panel-default">
      <div class="panel-heading">
        <strong>
          <span class="glyphicon glyphicon-th"></span>
          修改 <?php echo remove_junk(ucwords($e_user['name'])); ?> 密码
        </strong>
      </div>
      <div class="panel-body">
        <form action="edit_user.php?id=<?php echo (int)$e_user['id'];?>" method="post" class="clearfix">
          <div class="form-group">
                ...
                <input type="password" class="form-control" name="password" placeholder="输入用户新密码">
          </div>
          <div class="form-group clearfix">
                  <button type="submit" name="update-pass" class="btn btn-danger pull-right">修改</button>
          </div>
        </form>
      </div>
    </div>

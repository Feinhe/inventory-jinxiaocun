...
<?php
 if(isset($_POST['add_cat'])){
   ...
   if(empty($errors)){
      ...
      $sql .= " VALUES ('{$cat_name}')";
      if($db->query($sql)){
        $session->msg("s", "产品分类添加成功");
        redirect('categorie.php',false);
      } else {
        $session->msg("d", "抱歉！产品分类添加失败");
        redirect('categorie.php',false);
      }
   } else {
        ...
        <div class="panel-body">
          <form method="post" action="categorie.php">
            <div class="form-group">
                <input type="text" class="form-control" name="categorie-name" placeholder="分类名称">
            </div>
            <button type="submit" name="add_cat" class="btn btn-primary">增加分类</button>
        </form>
        </div>
      </div>
    </div>
